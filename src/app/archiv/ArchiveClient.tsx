    ...
    <>
      ...
      {fullscreenProtocol && fullscreenProtocol.file && (
        <div className="fixed inset-0 z-50 bg-slate-950 flex flex-col">
          {/* Minimalist top bar */}
          <div className="flex items-center justify-between px-4 py-3 bg-slate-950/90 backdrop-blur-sm border-b border-slate-800/50">
            <button
              onClick={closeFullscreen}
              className="inline-flex items-center gap-2 px-3 py-1.5 text-sm text-slate-400 rounded-md transition-all hover:text-cyan-400"
            >
            ...
            <a
              ...
              download
              className="inline-flex items-center gap-2 px-3 py-1.5 text-sm text-cyan-400 border border-cyan-400/30 rounded-md hover:bg-cyan-400/10 transition-all"
            >
              <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div className="relative">
              ...
              >
                <div className="inline-flex gap-5 relative px-10">
                  ...
                  <div className="absolute left-0 right-0 h-px bg-cyan-400/30 timeline-line-fade" style={{ top: "28px" }} />
                  ...
                  {filteredProtocols.map((protocol) => (
                    ...
                    >
                      ...
                      <div className="h-3 w-3 rounded-full bg-cyan-400/60 border-2 border-slate-900 shadow-[0_0_6px_rgba(103,232,249,0.4)] mb-3 relative z-10" />

                      {/* Protocol card - A4 proportioned */}
                      <div
                        className="w-full border border-slate-700/40 bg-slate-900/30 rounded-lg transition-all duration-300 cursor-pointer hover:border-cyan-400/30 hover:bg-slate-900/50"
                      ...
                      >
                        ...
                        {!protocol.file && (
                          <div className="flex items-center justify-center h-full p-3">
                            <h3 className="text-xs font-medium text-cyan-50 text-center line-clamp-2">
                              {protocol.title}
                            </h3>
                          </div>
                        )}
        ...
        {viewMode === "table" && filteredProtocols.length > 0 && (
          <div className="space-y-3">
            {[...filteredProtocols].reverse().map((protocol) => (
              <div
                key={protocol.slug}
                className="border border-slate-700/40 bg-slate-900/30 rounded-lg transition-all duration-300 cursor-pointer hover:border-cyan-400/30 hover:bg-slate-900/50"
                onClick={() => openFullscreen(protocol)}
              >
